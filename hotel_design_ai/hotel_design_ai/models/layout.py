...
from hotel_design_ai.utils.geometry import distance, create_room_box
...
class Layout:
    ...
    def get_room(self, room_id: int) -> Optional[Dict[str, Any]]:
        ...
        """
        return self.spatial_grid.rooms.get(room_id)
    ...
    def resize_room(
        self, 
        room_id: int, 
        new_dimensions: Tuple[float, float, float]
    ) -> bool:
        """
        Resize a room.
    ...
    def get_adjacency_graph(self) -> Dict[int, List[int]]:
        ...
        for room_id in self.spatial_grid.rooms:
    ...
    def are_adjacent(self, room_id1: int, room_id2: int) -> bool:
        ...
        return room_id1 in graph and room_id2 in graph[room_id1]
    ...
    def get_rooms_by_type(self, room_type: str) -> List[Dict[str, Any]]:
        ...
        """
        return [
            room_data for room_id, room_data in self.spatial_grid.rooms.items()
            if room_data['type'] == room_type
        ]
    
    def get_rooms_by_floor(self, floor: int) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            List[Dict]: List of room data
        """
        # Assuming standard floor height of 4m
        floor_height = 4.0
    ...
    def calculate_floor_areas(self) -> Dict[int, float]:
        """
        ...
        for room_id, room_data in self.spatial_grid.rooms.items():
            ...
            floor = int(z / 4.0)  # Assuming 4m floor height
    ...
    def find_path(self, start_room_id: int, end_room_id: int) -> Optional[List[int]]:
        ...
        """
        if start_room_id not in self.spatial_grid.rooms or end_room_id not in self.spatial_grid.rooms:
            return None
    ...
    def find_valid_position(
        ...
        if floor is not None:
            ...
            floor_height = 4.0  # Assuming 4m floor height
            z_min = floor * floor_height
            z_max = z_min + 0.1  # Only search at this z coordinate
        else:
            # Search entire building height
            z_min = 0
            z_max = self.spatial_grid.height - height
        
        # If near_room_id is specified, search near that room first
        if near_room_id is not None and near_room_id in self.spatial_grid.rooms:
            near_room = self.spatial_grid.rooms[near_room_id]
        ...
        # If structural grid is specified, use it for alignment
        if structural_grid:
            grid_x, grid_y = structural_grid
            
            # Search on grid intersections
            for z in np.arange(z_min, z_max + 0.1, self.spatial_grid.grid_size):
                for x in np.arange(0, self.spatial_grid.width - width + 0.1, grid_x):
                    for y in np.arange(0, self.spatial_grid.length - length + 0.1, grid_y):
                        if self.can_place_room(width, length, height, (x, y, z)):
                            return (x, y, z)
            
        # Exhaustive search if grid-based search fails or no grid specified
        for z in np.arange(z_min, z_max + 0.1, self.spatial_grid.grid_size):
            for x in np.arange(0, self.spatial_grid.width - width + 0.1, self.spatial_grid.grid_size):
                for y in np.arange(0, self.spatial_grid.length - length + 0.1, self.spatial_grid.grid_size):
                    if self.can_place_room(width, length, height, (x, y, z)):
                        return (x, y, z)
    ...
    def get_distance_between_rooms(self, room_id1: int, room_id2: int) -> Optional[float]:
        """
        ...
        """
        if room_id1 not in self.spatial_grid.rooms or room_id2 not in self.spatial_grid.rooms:
            return None
        
        room1 = self.spatial_grid.rooms[room_id1]
        room2 = self.spatial_grid.rooms[room_id2]
    ...
    def clone(self) -> 'Layout':
        ...
        """
        # Create a new spatial grid with the same dimensions
        new_grid = SpatialGrid(
            width=self.spatial_grid.width,
            length=self.spatial_grid.length,
            height=self.spatial_grid.height,
            grid_size=self.spatial_grid.grid_size
        )
        
        # Copy the grid data
        new_grid.grid = np.copy(self.spatial_grid.grid)
        
        # Copy the rooms data
        new_grid.rooms = {
            room_id: room_data.copy() 
            for room_id, room_data in self.spatial_grid.rooms.items()
        }
    ...
    def swap_rooms(self, room_id1: int, room_id2: int) -> bool:
        ...
        # Check if rooms can fit in each other's positions
        pos1 = room1['position']
        ...
        # If both placements succeed, return True
        ...
        # If either placement fails, try to restore original positions
    ...
    def get_room_boxes(self) -> Dict[int, Tuple[Tuple[float, float, float], Tuple[float, float, float]]]:
        """
        ...
        for room_id, room_data in self.spatial_grid.rooms.items():
            ...
            box = create_room_box(position, dimensions)
            room_boxes[room_id] = box
        
        return room_boxes
    
    def __str__(self) -> str:
        return f"Layout(name={self.name}, rooms={len(self.spatial_grid.rooms)})"
