...
class AdjacencyConstraint(Constraint):
    ...
    def __init__(
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or f"Adjacency({room_type1},{room_type2})")
...
class SeparationConstraint(Constraint):
    ...
    def __init__(
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or f"Separation({room_type1},{room_type2})")
        self.room_type1 = room_type1
        self.room_type2 = room_type2
...
class ExteriorAccessConstraint(Constraint):
    ...
    def __init__(
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or f"ExteriorAccess({room_type})")
    ...
    def evaluate(self, layout: SpatialGrid) -> float:
        ...
        rooms_with_access = sum(1 for room_id in rooms_of_type if room_id in exterior_rooms)
...
class FloorConstraint(Constraint):
    ...
    def __init__(
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or f"Floor({room_type},level={floor})")
        self.room_type = room_type
        self.floor = floor
    ...
    def evaluate(self, layout: SpatialGrid) -> float:
        ...
        for _, room_data in rooms_of_type:
            # Get floor from z coordinate, assuming 4m per floor
...
class ProximityConstraint(Constraint):
    ...
    def __init__(
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or f"Proximity({room_type1},{room_type2})")
        self.room_type1 = room_type1
        self.room_type2 = room_type2
...
class StructuralAlignmentConstraint(Constraint):
    """Constraint that rewards rooms aligning with the structural grid"""
    
    def __init__(
        self, 
        structural_grid: Tuple[float, float],
        weight: float = 1.0,
        name: Optional[str] = None
    ):
        """
        Initialize a structural alignment constraint.
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or "StructuralAlignment")
        self.grid_x, self.grid_y = structural_grid
    
    def evaluate(self, layout: SpatialGrid) -> float:
        """Evaluate how well rooms align with the structural grid"""
        if not layout.rooms:
        ...
        for room_id, room_data in layout.rooms.items():
            ...
            # Allow a small tolerance (5% of grid spacing)
            x_tolerance = 0.05 * self.grid_x
            y_tolerance = 0.05 * self.grid_y
            
            x_aligned = min(x % self.grid_x, self.grid_x - (x % self.grid_x)) < x_tolerance
            y_aligned = min(y % self.grid_y, self.grid_y - (y % self.grid_y)) < y_tolerance
            
            if x_aligned and y_aligned:
                aligned_count += 1
        
        return aligned_count / total_rooms
...
class NaturalLightConstraint(Constraint):
    ...
    def __init__(
        ...
        Args:
            ...
            name: Optional name for the constraint
        """
        super().__init__(weight, name or "NaturalLight")
    ...
    def evaluate(self, layout: SpatialGrid) -> float:
        ...
        # Count how many have exterior access
        rooms_with_light = sum(1 for room_id in rooms_needing_light if room_id in exterior_rooms)
...
class ConstraintSystem:
    ...
    def evaluate_weighted(self, layout: SpatialGrid) -> float:
        """
        Calculate a weighted average of all constraint scores.
...
def create_default_constraints(
    ...
    # Floor constraints
    floor_constraints = [
        FloorConstraint('entrance', 0, weight=3.0),  # Entrance on ground floor
        FloorConstraint('lobby', 0, weight=2.0),     # Lobby on ground floor
        FloorConstraint('parking', -1, weight=2.0),  # Parking in basement
    ]
    
    # Other constraints
    other_constraints = [
        ExteriorAccessConstraint('entrance', weight=3.0),
        ExteriorAccessConstraint('guest_room', weight=1.5),
        StructuralAlignmentConstraint(structural_grid, weight=1.0),
        NaturalLightConstraint(
            ['guest_room', 'restaurant', 'meeting_room', 'office'],
            weight=1.5
        ),
        ProximityConstraint('vertical_circulation', 'guest_room', 
                           max_distance=30.0, weight=1.0),
    ]
...
def from_adjacency_matrix(
    ...
    for i in range(len(room_types)):
        for j in range(i+1, len(room_types)):  # Only upper triangle to avoid duplicates
            value = adjacency_matrix[i][j]
            
            if value == 2:
                # Must be adjacent
                constraints.append(
                    AdjacencyConstraint(room_types[i], room_types[j], weight=base_weight * 2.0)
                )
            elif value == 1:
                # Preferred adjacent
                constraints.append(
                    AdjacencyConstraint(room_types[i], room_types[j], weight=base_weight)
                )
            elif value == -1:
                # Preferred separate
                constraints.append(
                    SeparationConstraint(room_types[i], room_types[j], weight=base_weight)
                )
            elif value == -2:
                # Must be separate
                constraints.append(
                    SeparationConstraint(room_types[i], room_types[j], weight=base_weight * 2.0)
                )
