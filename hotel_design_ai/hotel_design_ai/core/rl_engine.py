...
class RLEngine:
    ...
    def _get_state_representation(self, room_to_place: Room) -> torch.Tensor:
        """
        ...
        for room_id, room_data in self.spatial_grid.rooms.items():
            ...
            for x in range(grid_x, min(grid_x + grid_w, simplified_grid.shape[0])):
                for y in range(grid_y, min(grid_y + grid_l, simplified_grid.shape[1])):
                    for z in range(
                        grid_z, min(grid_z + grid_h, simplified_grid.shape[2])
                    ):
                        if (
                            0 <= x < simplified_grid.shape[0]
                            and 0 <= y < simplified_grid.shape[1]
                            and 0 <= z < simplified_grid.shape[2]
                        ):
        ...
        room_props = np.zeros(
            len(self.room_types) + 3
        )  # 3 basic props + one-hot encoding
        ...
        for room_id in self.fixed_elements:
            ...
            if room_data:
                ...
                for x in range(grid_x, min(grid_x + grid_w, simplified_grid.shape[0])):
                    for y in range(
                        grid_y, min(grid_y + grid_l, simplified_grid.shape[1])
                    ):
                        for z in range(
                            grid_z, min(grid_z + grid_h, simplified_grid.shape[2])
                        ):
                            if (
                                ...
                                idx = (
                                    z
                                    * simplified_grid.shape[0]
                                    * simplified_grid.shape[1]
                                    + y * simplified_grid.shape[0]
                                    + x
                                )
    ...
    def get_alternative_action(self, state: torch.Tensor, top_k: int = 5) -> int:
        ...
        with torch.no_grad():
            ...
            top_actions = torch.topk(
                q_values, min(top_k, self.action_dim)
            ).indices.numpy()
    ...
    def _check_vertical_alignment(self, layout: SpatialGrid) -> float:
        ...
        total_guest_rooms = sum(
            len(rooms)
            for key, rooms in position_groups.items()
            if key.startswith("guest_room")
        )
        aligned_guest_rooms = sum(
            len(rooms)
            for key, rooms in position_groups.items()
