...
class LayoutRenderer:
    ...
    def _draw_room_2d(
        ...
        if show_labels:
            ...
            # Try to get original_name - check for both direct key and nested in metadata
            if "original_name" in room_data:
                room_name = room_data["original_name"]
            elif (
                "metadata" in room_data
                and isinstance(room_data["metadata"], dict)
                and "original_name" in room_data["metadata"]
            ):
                room_name = room_data["metadata"]["original_name"]

            # If room has a long name, create a shorter version
            # if len(room_name) > 15:
            #     room_name = f"{room_data['type']}_{room_id}"
