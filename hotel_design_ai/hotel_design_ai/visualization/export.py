...
def export_to_revit(
    layout: SpatialGrid, filename: str, building_config: Dict[str, Any] = None
):
    """
    Export the layout to CSV format for import into Revit.
    ...
    with open(filename, "w", newline="") as csvfile:
        ...
        for room_id, room in layout.rooms.items():
            ...
            data = {
                ...
                "Level": int(z / building_config["floor_height"])
                + 1,  # Use floor height from building config
                "X_Meters": x,
                "Y_Meters": y,
                "Z_Meters": z,
                "Width_Meters": w,
...
""".replace(
                "REPLACE_WITH_ABSOLUTE_PATH_TO_CSV", filename
            )
        )
