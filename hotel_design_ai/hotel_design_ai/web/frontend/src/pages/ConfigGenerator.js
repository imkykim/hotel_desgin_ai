...
const ConfigGenerator = () => {
  ...
  const handleRequirementsReady = (requirementsResult) => {
    console.log("Requirements ready:", requirementsResult);

    // Store the requirements data for later use
    if (requirementsResult.success && requirementsResult.requirements) {
      setRequirementsData({
        data: requirementsResult.requirements,
        program_id: requirementsResult.program_id,
      });
    }
  };
  ...
  const generateConfiguration = async (requirementsResult = null) => {
    ...
    try {
      ...
      let requirementsData = { note: "Generated by Chat2Plan" };
      let programId = "default";
      let requirementsFilename = "default_requirements.json";

      // If we have requirements data from the function parameter, use that
      if (requirementsResult && requirementsResult.success) {
        requirementsData = requirementsResult.requirements || requirementsData;
        programId = requirementsResult.program_id || programId;
        requirementsFilename = `${programId}.json`;
      }
      // Otherwise, try to fetch requirements if we have a session ID
      else if (sessionId) {
        try {
          const fetchedRequirements = await exportRequirements(sessionId);
          if (fetchedRequirements.success) {
            requirementsData =
              fetchedRequirements.requirements || requirementsData;
            programId = fetchedRequirements.program_id || programId;
            requirementsFilename = `${programId}.json`;
          }
        } catch (reqErr) {
          console.error("Error fetching requirements:", reqErr);
          // Continue with default requirements if there's an error
        }
      }
  ...
  const proceedToInteractive = () => {
    // Use the building ID from the generated config
    const buildingId =
      generatedConfigs?.building_envelope?.filename.replace(".json", "") ||
      "default";

    // Use the program ID from the generated requirements or default if not available
    const programId =
      generatedConfigs?.hotel_requirements?.program_id || "default";
