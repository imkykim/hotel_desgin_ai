...
const ConfigGenerator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!requirementsReady) {
      setError(
        "Please complete the Chat2Plan process to generate hotel requirements first."
      );
      return;
    }
    ...
    try {
      // Only generate the building envelope config
      ...
      const safe_name = formData.hotel_name.toLowerCase().replace(/\s+/g, "_");
      ...
      setGeneratedConfigs({
        ...
        hotel_requirements: {
          filename: `${programId}.json`,
          data: { note: "Generated by Chat2Plan" },
        },
      });
    } catch (err) {
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                {requirementsReady ? (
                  <div className="requirements-status success">
                    <p>
                      ✓ Hotel requirements generated successfully! ID:{" "}
                      {programId}
                    </p>
                  </div>
                ) : (
                  <div className="requirements-status pending">
                    <p>
                      ⚠️ Please complete the Chat2Plan process to generate
                      requirements
                    </p>
                  </div>
                )}
            ...
            <div className="form-actions">
              <button
                ...
                disabled={loading || !requirementsReady}
