...
class LayoutMetrics:
    ...
    def circulation_efficiency(self) -> Dict[str, float]:
        ...
        vertical_circulation_rooms = [
            room for room in circulation_rooms 
    ...
    def adjacency_satisfaction(self, adjacency_preferences: Dict[str, List[str]]) -> float:
        """
    ...
    def natural_light_access(self) -> float:
        ...
        natural_light_types = [
            'guest_room', 'restaurant', 'meeting_room', 'lobby', 'office', 'ballroom'
        ]
    ...
    def department_clustering(self, room_departments: Dict[str, str]) -> float:
        ...
        for dept, room_ids in departments.items():
            ...
            for center in centers:
                if other_centers:
                    min_dist = min(manhattan_distance(center, other_center) 
    ...
    def vertical_stacking(self) -> float:
        ...
        for room_type in stacking_types:
            ...
            stacked_rooms = sum(len(group) for group in position_groups.values() if len(group) > 1)
    ...
    def evaluate_all(
        ...
        if adjacency_preferences:
            metrics['adjacency_satisfaction'] = self.adjacency_satisfaction(adjacency_preferences)
        
        if room_departments:
            metrics['department_clustering'] = self.department_clustering(room_departments)
        ...
        if 'circulation_efficiency' in metrics and 'circulation_efficiency' in metric_weights:
            ...
            corridor_score = max(0, 1 - (circ_metrics['avg_corridor_length'] / max_corridor))
...
def calculate_egress_capacity(layout: SpatialGrid) -> Dict[str, Any]:
    """
    ...
    for room_id, load in occupant_loads.items():
        ...
        # Determine floor (assuming 4m floor height)
        floor = int(z / 4.0)
...
def calculate_program_compliance(
    layout: SpatialGrid, 
    program_requirements: Dict[str, Dict[str, float]]
) -> Dict[str, Any]:
    """
    ...
    for room_type, rooms in rooms_by_type.items():
        total_area = sum(room['dimensions'][0] * room['dimensions'][1] for room in rooms)
    ...
    total_required = sum(req['area'] if isinstance(req, dict) else req 
