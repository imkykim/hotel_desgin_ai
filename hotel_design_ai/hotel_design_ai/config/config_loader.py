...
for directory in [DATA_DIR, BUILDING_DIR, CONSTRAINTS_DIR, PROGRAM_DIR, 
                 ROOM_TYPES_DIR, TEMPLATES_DIR, RL_DIR]:
...
def get_constraints_by_type(constraint_type: str) -> List[Dict[str, Any]]:
    ...
    if not type_constraints:
        ...
        type_constraints = [c for c in all_constraints if c.get("type") == constraint_type]
...
def create_room_objects_from_program() -> List[Dict[str, Any]]:
    ...
    for department_key, department in program.items():
        for space_key, space in department.items():
            ...
            natural_light = space.get("requires_natural_light", requirements.get("natural_light", False))
            adjacencies = space.get("requires_adjacency", requirements.get("preferred_adjacencies", []))
...
if __name__ == "__main__":
    ...
    for i in range(min(3, len(rooms))):
        print(f"Room {i+1}: {rooms[i]['name']} - {rooms[i]['width']:.1f}m x {rooms[i]['length']:.1f}m")
